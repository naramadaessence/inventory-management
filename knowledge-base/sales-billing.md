...
- `supabase-scripts/clear-18000-sales-data.sql` is a one-off data cleanup script for removing existing 18000-total sales without changing product stock.
...
- The one-off 18000 cleanup script intentionally does not call `delete_sale`, because that client request is sales-data-only cleanup with stock left unchanged.
